...
export const WorkHoursGrid: React.FC<propsWorkHoursGrid> = (
  props: propsWorkHoursGrid
) => {
  const classes = useStyles()
  const [selectedDepOnChart, SetselectedDepOnChart] = useState("")
  const [
    workHoursViolsTableJSON,
    setWorkHoursViolsTableJSON,
  ] = useState<object>({})
  const [
    workHoursViolsByDepJSON,
    setWorkHoursViolsByDepJSON,
  ] = useState<object>({})
  const [
    workHoursViolsByEmpsDepJSON,
    setWorkHoursViolsByEmpsDepJSON,
  ] = useState<object>({})
    ...
    <React.Fragment>
          ...
          <Grid item xs={12} md={12}>
            <Card className={classes.paper2}>
              <WorkHoursByDepChart
                ...
                setExportJSON={setWorkHoursViolsByDepJSON}
              />
            </Card>
          </Grid>
          <Grid item xs={12} md={12}>
            <Card className={classes.paper2}>
              <WorkHoursByEmpChart
                depName={selectedDepOnChart}
                setExportJSON={setWorkHoursViolsByEmpsDepJSON}
              />
            </Card>
          </Grid>
        </Grid>
      ...
      <Grid item xs={12} md={6}>
        <Card className={classes.paper}>
          <WorkHoursTable setExportJSON={setWorkHoursViolsTableJSON} />
        </Card>
      </Grid>
    </React.Fragment>
  )
